...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        // Skip the main folder link (/songs/) and only take subfolders
        if (e.href.includes("/songs/") && !e.href.endsWith("/songs/")) {
            let folder = e.href.split("/").slice(-2)[1];

            try {
                // Fetch metadata
                let resInfo = await fetch(`http://127.0.0.1:5500/songs/${folder}/info.json`);
                if (!resInfo.ok) {
                    console.warn(`No info.json found in folder: ${folder}`);
                    continue;
                }

                let info = await resInfo.json();

                // Use the real folder name instead of hardcoded "cs"
...
async function main() {
    ...
    // Add event listener previous and next
